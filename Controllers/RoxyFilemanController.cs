...
{
    [Authorize(Roles = "Admin,Employee")]    // Important: uncomment this in production to prevent huge security risk!
    ...
    public class RoxyFilemanController : Controller
    {
        ...
        public RoxyFilemanController(IHostEnvironment env)
        {
            // Setup CMS paths to suit your environment (we usually inject settings for these)
            _systemRootPath = env.ContentRootPath;
            _tempPath = _systemRootPath + "\\wwwroot\\dll\\Temp";
            _filesRootPath = "/wwwroot/dll/Content";
            _filesRootVirtual = "/dll/Content";
